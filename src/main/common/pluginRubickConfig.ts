...
export const PLUGIN_UI_SETTINGS_FILE = 'rubick-plugin-ui-settings.json';

/** 不读写文件的系统插件（无独立包目录、无对应 market name） */
const SKIP_NAMES = new Set(['rubick-system-super-panel']);

export type PluginRubickConfig = {
  autoDetach?: boolean;
  /** 独立窗口顶栏是否始终显示搜索框（否则仍按 subInput 是否有内容切换） */
  detachAlwaysShowSearch?: boolean;
};

type SettingsMap = Record<string, PluginRubickConfig>;
...
function cloneRow(v: unknown): PluginRubickConfig {
  if (!v || typeof v !== 'object' || Array.isArray(v)) return {};
  const o = v as PluginRubickConfig;
  return {
...
export function readPluginRubickConfigSync(name: string): PluginRubickConfig {
...
export function writePluginRubickConfigSync(
  name: string,
  patch: Partial<PluginRubickConfig>
): boolean {
...
export function flipPluginAutoDetachSync(name: string): boolean {
  const cur = readPluginRubickConfigSync(name).autoDetach === true;
  const next = !cur;
  writePluginRubickConfigSync(name, { autoDetach: next });
  return next;
}

export function flipPluginDetachAlwaysShowSearchSync(name: string): boolean {
  const cur = readPluginRubickConfigSync(name).detachAlwaysShowSearch === true;
  const next = !cur;
  writePluginRubickConfigSync(name, { detachAlwaysShowSearch: next });
  return next;
}
